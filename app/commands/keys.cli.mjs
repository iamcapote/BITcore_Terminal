...
export async function executeKeys(options = {}) {
    ...
    const output = cmdOutput ?? console.log;
    const error = cmdError ?? console.error;
...
async function handleSet(service, value, flags, output, error) {
    if (!service) {
        error('Usage: /keys set <service> [options]');
...
async function handleTest(output, error) {
    output('Testing configured credentials...');
    ...
    output('Credential tests completed.');
    return { success: true, keepDisabled: false };
}
