...
export class UserManager {
  ...
  async deleteUser(username) {
    ...
    if (username === 'public') {
      throw new Error('Cannot delete the public user');
    }

    // Prevent admins from deleting themselves
    if (username === this.currentUser.username) {
      throw new Error('Cannot delete the currently logged-in admin user');
    }
    ...
    try {
      ...
      // Get user data to check if trying to delete the last admin
      const userData = JSON.parse(await fs.readFile(userPath, 'utf8'));
      ...
      console.log(`[Auth] User ${username} deleted successfully`);
      return true;
    } catch (error) {
      if (error.code === 'ENOENT') {
        throw new Error(`User ${username} does not exist`);
      }
      console.error(`[Auth] Error deleting user ${username}: ${error.message}`);
